...
\email{f.kammueller@mdx.ac.uk|bianca.lutz@campus.tu-berlin.de}
}
\maketitle
\begin{abstract}
We provide a model in the Isabelle Infrastructure framework of the recently published German
Corona-virus warning app (CWA). The app supports breaking infection chains by informing users
...
continuously changing Ephemeral Ids (EphIDs) improves the anonymity.
\end{abstract}

\section{Introduction}
\label{sec:intro}
The German Chancellor Angela Merkel has strongly supported the publication of
the mobile phone Corona-virus warning app (CWA; \cite{cwa:github}) by publicly proclaiming that
``this App deserves your trust'' \cite{bundes:20}. Many millions of mobile phone users
...
highly anonymized identifiers (ids)
%so called ``Ephemeral Ids''
via Bluetooth and store those ids of people in close proximity.
...
it allows revealing weak points of the architecture, show that the measures that have
...
The contributions of this paper are (a) formal re-engineering of CWA
...
(c) a formal definition of a security refinement process that allows improving a system
based on attacks found by the attack tree analysis and (d) an application of the refinement to
improve security of the CWA specification.
...
framework is introduced and illustrated on CWA (Section \ref{sec:ref})
...
Isabelle (sources available \cite{kam:20gitat}). 
...
%the main
core principles of the DP-3T proposal. This API is not only used in 
...
In terms of attack scenarios, we focus on, what might be classified as deanonymization attacks: Tracking a device (see \cite{dp3t:psre}[p9], \cite{pepppt:dp3tana}[p8]) and identifying infected individuals (\cite{dp3t:psre}[p5], \cite{pepppt:dp3tana}[p9]).
...
\vspace{-.5cm}
  \begin{center}
  \includegraphics[scale=.45]{theory_structure.pdf}
\end{center}
\vspace{-.5cm}
\caption{Generic Isabelle Infrastructure framework applied to CWA.}
...
Using a generic state transition relation $\to$, Kripke structures
are defined as a set of states reachable by $\to$ from an initial
state set, for example,
\begin{ttbox}
Kripke \{t. \ttexists i \ttin I. i \ttrelIstar t\} I
\end{ttbox}
where $\to^*$ is the reflexive transitive closure over $\to$.
\item {CTL statements:}\\ 
...
An abstract attack tree \texttt{A} of type \texttt{\ttalpha\ attree} over an arbitrary state type \texttt{\ttsigma}
may be refined by spelling out the attack steps until a valid attack is reached:
...
here in the stepwise system refinements.
\end{itemize}


\subsubsection{Instantiation of Framework}
The formal model of CWA uses the Isabelle Infrastructure framework instantiating it 
...
for CWA, express relevant and interesting properties, and conduct
interactive proofs (with the full support of the powerful and highly automated proof support
of Isabelle).
%The CWA theory itself is an adaptation of the Infrastructure theory of the 
...
All Isabelle sources are available online \cite{kam:20gitat}.
...
refinement (or datatype refinement). The definition allows proving property preservation results 
...
\section{Modeling and analyzing CWA}
\label{sec:model}
...
\end{ttbox}
In the current application of the framework to the CWA case study, 
...
just a natural number of type \texttt{nat} mapped by the constructor \texttt{Efid} to the new datatype
\texttt{efid} but will be refined to actually represent lists of Ephemeral Ids later
...
In CWA, the initial values for the \texttt{igraph} components use
...
The local policies definition for CWA, simply permits all actions to
...
\end{ttbox}  
For CWA, the initial infrastructure contains the graph \texttt{ex\_graph}
...
\end{ttbox}
The predicate \texttt{identifiable} is used to express as the global policy `Eve cannot deanonymize an Ephemeral
...
The state transition relation uses the syntactic infix notation \texttt{I \ttrel\, I'}  to denote that
infrastructures \texttt{I} and \texttt{I'} are in this relation. 
We will use this state transition to define the dynamic behaviour of the system including actors' movements and
the evolution of their knowledge. This allows representing attacks as sequences of state transitions leading to
states in which the global policy is violated. 
To give an intuitive motivating idea before the formal presentation in Section \ref{sec:ana}: the attack will
consist of the state transitions of Eve getting available EphIds at the location pub, Alice moving to shop, Eve
following her and again getting the EphIds at shop. 

So, one relevant rule for the state transition is the one for \texttt{move} that defines the state transition relation
expressing state changes when actors move across the infrastructure.
%whose structure resembles the previous one.
\begin{ttbox}
...
%To give an impression of this definition,
Another action relevant for the deanonymization attack describes how an actor gets information.
This is defined by the rule of the state transition for the action get. Initially, this rule
expresses that an actor that resides at a location \texttt{l} (\texttt{a \ttatI\ l})
...
\end{ttbox}
We can simply apply the Correctness theorem \texttt{AT\_EF} \cite{kam:18b,kam:18a}
\begin{ttbox}
\ttvdash A \ttImp attack A = (I,s)
\ttImp Kripke \{s . \ttexists i \ttin I. (i \ttrelIstar s) \} I \ttvdash EF s
\end{ttbox}  
to immediately prove this CTL statement. This application of the meta-theorem 
...
This relation, called \texttt{refinement}, is typed as a relation over triples --
...
\texttt{refinement} is a function of type $\sigma' \Rightarrow \sigma$ 
...
instead of writing \texttt{refinement} in prefix manner. This nicer infix
...
types: within a HOL formula, for example, within the below $\forall $\texttt{s' \ttin\ states K'.}\dots,
only the implication $\ttimp$ can be used.
\begin{ttbox}
...
under ${\mathcal E}$ must be reached within $0$ and $n$ steps.
...
represents function image, i.e.,  $\{\ttecal(x) \mid x \in \texttt{init K'}\} $.
\begin{ttbox}
...
structures entails property preservation and makes it possible 
...
allows explicitly formalizing such sophisticated concepts like reachability.
...
have the simpler one-step proviso %but still need 
and also reachability from initial states 
we provide a slightly weaker version of \texttt{strong\_mt}.
%that pre-supposes reachability of \texttt{s} from initial states.
\begin{ttbox}
{\bf{theorem}} strong_mt':  
...
the properties of a system are given by the set of its event traces. Now, a refinement
of the system is given by another system that has a subset of the event traces of the 
former one.
Although the principal idea is similar, we greatly extend it since our notion
additionally incorporates data refinement. Since we include a state map 
...
constituents in an \texttt{efidlist} (the root $SK_t$ of type \texttt{efid}); \texttt{efids\_index} giving the second
component of type \texttt{nat}, that is, the index of the current \texttt{efid}; \texttt{efids\_inc\_ind} applied to an
\texttt{efidlist} increments the index; \texttt{efids\_cur} returning the current \texttt{efid}
from the list and \texttt{efids\_list} for the entire list (the third component of type \texttt{efid list}).
...
and the knowledge set, remain the same and still operate on the simple type \texttt{efid}.
%\begin{ttbox}
%{\bf datatype} igraph =
%         Lgraph (location \tttimes location)set
%                 location \ttfun identity set
...
same attack tree as in the abstract model: if Bob moves from pub to shop\footnote{that is, if he moves alone:
  if all others from pub go to shop with him his anonymity remains intact.}, he is vulnerable to
being identifiable as long as he does not change the current \texttt{efid}. So, if Eve moves to
the shop as well and performs a get before Bob does a put, then Eve's knowledge sets permit identifying
...
by adding an increment (highlighted in the code snippet in bold letters)
